...
def click_time():
    """
    Set the click time to as low as one second
    """
    
    click_reg = r"(?P<time>[\d]+)"
    get_time = set_time.get()
    ac.Autoclicker.clicking_time = int(re.search(click_reg, get_time).group())
...
set_time_button = tk.Button(window, text="Set Time", command=click_time, bg="green")
